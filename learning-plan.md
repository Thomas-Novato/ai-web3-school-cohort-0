# 个人学习计划

## 学习目标

基于我的背景（AI 有基础，Web3 有基础，会 C++ 编程），目标是：
1. 掌握 AI × Web3 开发技能
2. 完成 Hackathon 项目
3. 输出学习内容

## 学习路径

### 阶段一：基础补充（2-3 周）

#### AI 基础强化
- [x] [LLM](https://aiweb3.school/zh/handbook/ai/llm/) - ✅ 已完成 (5/18)
- [x] [Prompt](https://aiweb3.school/zh/handbook/ai/prompt/) - ✅ 已完成 (5/19)
- [x] [Context](https://aiweb3.school/zh/handbook/ai/context/) - ✅ 已完成 (5/20)
- [x] [Agent](https://aiweb3.school/zh/handbook/ai/agent/) - ✅ 已完成 (5/21)
- [ ] [RAG](https://aiweb3.school/zh/handbook/ai/rag/) - 检索增强生成
- [ ] [Frameworks](https://aiweb3.school/zh/handbook/ai/frameworks/) - LangChain/LangGraph

#### Web3 基础强化
- [x] [Network](https://aiweb3.school/zh/handbook/web3/network/) - 区块链网络基础 ✅ (5/27)
- [x] [Wallet](https://aiweb3.school/zh/handbook/web3/wallet/) - 钱包和签名 ✅ (5/29)
- [ ] [Smart Contract](https://aiweb3.school/zh/handbook/web3/smart-contract/) - 智能合约
- [ ] [Account Abstraction](https://aiweb3.school/zh/handbook/web3/account-abstraction/) - 账户抽象
- [ ] [Dev Stack](https://aiweb3.school/zh/handbook/web3/dev-stack/) - 开发工具链

#### 编程技能补充
- [ ] Python 基础（AI 开发常用）
- [ ] JavaScript/TypeScript 基础（Web3 开发常用）
- [ ] 开发环境搭建

### 阶段二：AI × Web3 交叉（3-4 周）

#### Bridge 核心概念
- [x] [Chain-aware Context](https://aiweb3.school/zh/handbook/bridge/chain-aware-context/) - 链上状态进入 Agent ✅ (5/30)
- [x] [Web3 Tool Use](https://aiweb3.school/zh/handbook/bridge/web3-tool-use/) - Agent 调用 Web3 工具 ✅ (6/3)
- [x] [Agent Workflow](https://aiweb3.school/zh/handbook/bridge/agent-workflow/) - 工作流设计 ✅ (6/1)
- [x] [Agent Wallet](https://aiweb3.school/zh/handbook/bridge/agent-wallet/) - Agent 权限管理 ✅ (5/29, 配套 MVP: Approval Tracker)
- [x] [Machine Payment](https://aiweb3.school/zh/handbook/bridge/machine-payment/) - 机器支付 ✅ (5/31)

#### 安全和隐私
- [ ] [AI Security](https://aiweb3.school/zh/handbook/bridge/ai-security/) - Prompt Injection、权限隔离
- [ ] [AI Privacy](https://aiweb3.school/zh/handbook/bridge/ai-privacy/) - 隐私边界
- [ ] [Web3 Security](https://aiweb3.school/zh/handbook/web3/security/) - 合约安全

### 阶段三：项目实践（4-6 周）

#### 选择方向
根据兴趣选择一个前沿探索方向：
- [ ] [Agentic Commerce](https://aiweb3.school/zh/handbook/tracks/agentic-commerce/) - 智能体商业
- [ ] [Wallet / Permission](https://aiweb3.school/zh/handbook/tracks/wallet-permission/) - 钱包与权限
- [ ] [AI Security](https://aiweb3.school/zh/handbook/tracks/ai-security/) - AI 安全
- [ ] [Governance](https://aiweb3.school/zh/handbook/tracks/governance/) - 治理
- [ ] [Dev Tooling](https://aiweb3.school/zh/handbook/tracks/dev-tooling/) - 开发工具

#### 项目开发
- [ ] 确定项目方向和需求
- [ ] 设计系统架构
- [ ] 实现核心功能
- [ ] 测试和优化
- [ ] 文档和演示

#### Hackathon 准备
- [ ] 组队或独立参赛
- [ ] 完善项目
- [ ] 准备 Demo 和 Pitch
- [ ] 提交作品

## 每日学习节奏

### 工作日（1-2 小时）
- **理论学习**: 30-45 分钟（阅读 Handbook、看视频）
- **动手实践**: 30-45 分钟（写代码、做实验）
- **笔记整理**: 15-30 分钟（记录学习内容）

### 周末（2-4 小时）
- **深度学习**: 1-2 小时（完整章节、复杂概念）
- **项目实践**: 1-2 小时（开发、调试）
- **复盘总结**: 30 分钟（周总结、计划调整）

## 学习方法

### 最小路径（每日必做）
- 阅读 1 个 Handbook 章节
- 完成 1 个小实验或代码练习
- 写 1 篇 daily note

### 推荐路径（有时间时）
- 深入研究相关资料
- 尝试更复杂的实验
- 输出技术文章

### 挑战路径（周末或充裕时）
- 完成完整的项目模块
- 参与社区讨论
- 贡献 Handbook feedback

## 输出计划

### 学习笔记
- 每日学习记录（daily/）
- 任务完成记录（tasks/）
- 实验代码（experiments/）

### 技术内容
- 每周总结文章
- 技术教程
- 项目文档

### Handbook Feedback
- 记录学习中的问题
- 提出改进建议
- 补充案例和资料

## 里程碑

- [ ] Week 1-2: 完成 AI 基础模块
- [ ] Week 3-4: 完成 Web3 基础模块
- [ ] Week 5-6: 完成 Bridge 核心概念
- [ ] Week 7-8: 确定项目方向，开始开发
- [ ] Week 9-10: 完成项目核心功能
- [ ] Week 11-12: 项目完善，准备 Hackathon

## 调整记录

- 2026-05-18: 初始化学习计划
